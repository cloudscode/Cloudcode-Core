...
<form role="form" class="form-horizontal" id="myFormId" action="${request.getContextPath()}/groups/createMenu" method="post">
...
$(function () {
   if('${entityAction}' =='update'){
	   $('#updateButton').click( function() {
	      		   ...
	      		   $.ajax({
				        url: '${request.getContextPath()}/groups/'+$("#oid").val()+'/updateMenu',
				        type: 'post',
				        dataType: 'json',
	    ...
	    $('#updateButton').click( function() {
			    ...
			    $.ajax({
			        url: '${request.getContextPath()}/groups/createMenu',
			        type: 'post',
			        dataType: 'json',
			        data: $('form#myFormId').serialize(),
			        success: function(data) {
