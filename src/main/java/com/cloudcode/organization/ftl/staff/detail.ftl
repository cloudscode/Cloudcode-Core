...
<form role="form" class="form-horizontal" id="myFormId" action="${request.getContextPath()}/staffs/createMenu" method="post">
...
$(function () {
   if('${entityAction}' =='update'){
	   $('#updateButton').click( function() {
	      		   ...
	      		   $.ajax({
				        url: '${request.getContextPath()}/staffs/'+$("#oid").val()+'/updateMenu',
				        type: 'post',
				        dataType: 'json',
	    ...
	    $('#updateButton').click( function() {
			    ...
			    $.ajax({
			        url: '${request.getContextPath()}/staffs/createMenu',
			        type: 'post',
			        dataType: 'json',
			        data: $('form#myFormId').serialize(),
			        success: function(data) {
...
function zTreeOnDblClick(event, treeId, treeNode) {
	...
	var setting = {
			...
			async: {
		        ...
		        //返回的JSON数据的名字
		        dataName: 'treeNodes',
		        url:'${request.getContextPath()}/staffs/queryDataTreeByPid'
		       }
		};		
