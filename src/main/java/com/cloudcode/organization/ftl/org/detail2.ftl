  ...
  <div class="form-group">
   ...
   <div class="col-sm-4">
        ...
        <span id="node_span" xtype="selectTree"
							config="name: 'idCode' , tableName : 'ORG_ORGANIZATION' , url : '../orgs/queryTreeList' , params : {isNoLeaf : true} "></span>
	</div>
    </div>
   <div class="form-group">
   ...
   <div class="form-group">
    	<label for="inputEmail3" class="col-sm-2 control-label">集团</label>
	    <div class="col-sm-10">
	        <span id="node_span" xtype="selectTree"
							config="name: 'groupId' , tableName : 'ORG_GROUP' , url : '../groups/queryTreeList' , params : {isNoLeaf : true},required :true "></span>
	    </div>
  </div>
  <div class="form-group">
...
requirejs(['jquery','jquery','Request','jqueryui','main','text','select','date','radio','checkbox','textarea','password','ckeditor','button','validation','Request','combobox'], function( $, jQuery,Request ) {
	...
	page.save=function () {
		$.cc.validation.check('form', function(formData) {
			...
			var url='${request.getContextPath()}/orgs/createOrganization';
			if('${entityAction}' =='update'){
			  url='${request.getContextPath()}/orgs/'+$("[name='id']").val()+'/updateOrganization'
			}
			Request.request(url, {
				data : formData,
