...
<form role="form" class="form-horizontal" id="myFormId" action="${request.getContextPath()}/orgs/createMenu" method="post">
  <div class="form-org">
    <label for="inputEmail3" class="col-sm-2 control-label">名称</label>
	    <div class="col-sm-4">
	      <input type="text" name="text" class="form-control" id="text" placeholder="名称">
	    </div>
     <label for="inputPassword3" class="col-sm-2 control-label">集团</label>
     <div class="col-sm-4">
...
$(function () {
   if('${entityAction}' =='update'){
	   $('#updateButton').click( function() {
	      		   ...
	      		   $.ajax({
				        url: '${request.getContextPath()}/orgs/'+$("#oid").val()+'/updateMenu',
				        type: 'post',
				        dataType: 'json',
	    ...
	    $('#updateButton').click( function() {
			    ...
			    $.ajax({
			        url: '${request.getContextPath()}/orgs/createMenu',
			        type: 'post',
			        dataType: 'json',
			        data: $('form#myFormId').serialize(),
			        success: function(data) {
