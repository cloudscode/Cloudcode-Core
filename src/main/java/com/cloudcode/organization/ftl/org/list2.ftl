...
$(function(){
    if ($.fn.jqGrid){
        grid = $("#jqGrid01").jqGrid({
            ...
            ExpandColumn: 'name',
            url:"${request.getContextPath()}/orgs/queryTree",
            datatype: "json",
            height: 250,
            rowNum: 10,
...
requirejs(['jquery','Dialog','Request','jqueryui','main','text','select','date','radio','checkbox','textarea','password','ckeditor','button','validation','Request','combobox'], function( $, Dialog,Request ) {
	...
	$( "#add" ).click(function(){
		options.url='${request.getContextPath()}/orgs/collection/toDetail';
		Dialog.open(options);
	});
	$( "#edit" ).click(function(){
		var id; 
		...
		if(null != id && id.toString() != null && id.toString() != ""){
			    options.url='${request.getContextPath()}/orgs/'+id+'/toDetail';
				Dialog.open(options);
		}else{
