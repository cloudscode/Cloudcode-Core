...
$(function(){
    if ($.fn.jqGrid){
        grid = $("#jqGrid01").jqGrid({
            url:"${request.getContextPath()}/systemfile/query",
            datatype: "json",
            height: 250,
            rowNum: 10,
...
$( "#openwindow" ).click(function(){
	$( "#divInDialog" ).dialog({
		 ...
		 open: function(event, ui) {
 		 	  $(this).load('${request.getContextPath()}/systemfile/create');
	 	 },	   
	...
	id = grid.jqGrid('getGridParam','selarrrow');
	  if(id.toString() != null && id.toString() != ""){
			...
			divInDialogs =$( "#divInDialog" ).dialog({
			 	 ...
			 	 width:800,
				open: function(event, ui) {
					$(this).load('${request.getContextPath()}/systemfile/'+id+'/update');
			  },	   
		    close: function (event, ui) {  
		       grid.trigger("reloadGrid");
		    }  	   
...
$( "#delete" ).click(function(){
		var id;
	     ...
	     if(id.toString() != null && id.toString() != ""){
		    $.ajax({
			        url: '${request.getContextPath()}/systemfile/deleteAll',
			        type: 'post',
			        dataType: 'json',
