...
<script src="/cccommon/static/jquery/ui/bootstrap/third-party/jqGrid/jqGrid/js/grid.treegrid.js" type="text/javascript"></script>
<!--end jqGrid-->
...
$(function(){
    if ($.fn.jqGrid){
        grid = $("#jqGrid01").jqGrid({
            ...
            ExpandColumn: 'name',
            url:"${request.getContextPath()}/categories/queryTree",
            datatype: "json",
            height: 250,
            rowNum: 10,
            prmNames : {PageRange:{page:"page",rows:"rows"}  },
            rowList: [10,20,30],
            colNames:['Id', '名称','父节点','动作'],
            colModel:[
...
$( "#openwindow" ).click(function(){
	/*$( "#divInDialog" ).dialog({
		...
		open: function(event, ui) {
  			// $('#divInDialogContent').load('${request.getContextPath()}/categories/create', function() {
   				
 		 	// });
 		 	  $(this).load('${request.getContextPath()}/categories/create');
	 	 },	   
...
//	var parentBody = $('#sys');
	...
	var dia=$('<div id="divInDialog" >测试</div>');
	$(parentBody).append(dia);	
	$('#divInDialog',window.parent.document).dialog({
	 	// modal: true,
	 	 width:800,
		open: function(event, ui) {
  			// $('#divInDialogContent').load('${request.getContextPath()}/categories/create', function() {
	...
	id = grid.jqGrid('getGridParam','selarrrow');
	  if(id.toString() != null && id.toString() != ""){
			...
			divInDialogs =$( "#divInDialog" ).dialog({
			 	 ...
			 	 width:800,
				open: function(event, ui) {
		  			$('#divInDialog').load('${request.getContextPath()}/categories/'+id+'/update', function() {
		 		 	 });
			  },	   
		    close: function (event, ui) {  
		       grid.trigger("reloadGrid");
...
$( "#delete" ).click(function(){
		var id;
	     ...
	     if(id.toString() != null && id.toString() != ""){
		    $.ajax({
			        url: '${request.getContextPath()}/categories/deleteAll',
			        type: 'post',
			        dataType: 'json',
